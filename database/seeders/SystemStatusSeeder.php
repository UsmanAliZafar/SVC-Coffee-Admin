<?php

namespace Database\Seeders;

use App\Models\SystemStatus;
use Illuminate\Database\Seeder;
use Illuminate\Support\Str;

class SystemStatusSeeder extends Seeder
{
    /**
     * Run the database seeds.
     */
    public function run(): void
    {
        $allStatuses = [
            // ============================================
            // CATEGORIES MODULE STATUSES
            // ============================================
            [
                'module' => 'categories',
                'name' => 'Active',
                'key_code' => 'CATEGORY_ACTIVE',
                'slug' => 'active',
                'color' => '#ffffff',
                'bg_color' => '#28a745',
                'icon' => 'bi-check-circle-fill',
                'description' => 'Category is active and visible on the website',
                'order' => 1,
                'is_active' => true,
                'is_default' => true,
                'is_final' => false,
            ],
            [
                'module' => 'categories',
                'name' => 'Inactive',
                'key_code' => 'CATEGORY_INACTIVE',
                'slug' => 'inactive',
                'color' => '#ffffff',
                'bg_color' => '#6c757d',
                'icon' => 'bi-dash-circle-fill',
                'description' => 'Category is inactive and hidden from the website',
                'order' => 2,
                'is_active' => true,
                'is_default' => false,
                'is_final' => false,
            ],
            [
                'module' => 'categories',
                'name' => 'Draft',
                'key_code' => 'CATEGORY_DRAFT',
                'slug' => 'draft',
                'color' => '#856404',
                'bg_color' => '#fff3cd',
                'icon' => 'bi-pencil-square',
                'description' => 'Category is in draft mode',
                'order' => 3,
                'is_active' => true,
                'is_default' => false,
                'is_final' => false,
            ],

            // ============================================
            // PRODUCTS MODULE STATUSES
            // ============================================
            [
                'module' => 'products',
                'name' => 'Active',
                'key_code' => 'PRODUCT_ACTIVE',
                'slug' => 'active',
                'color' => '#ffffff',
                'bg_color' => '#28a745',
                'icon' => 'bi-check-circle-fill',
                'description' => 'Product is active and available for purchase',
                'order' => 1,
                'is_active' => true,
                'is_default' => true,
                'is_final' => false,
                'send_notification' => true,
            ],
            [
                'module' => 'products',
                'name' => 'Draft',
                'key_code' => 'PRODUCT_DRAFT',
                'slug' => 'draft',
                'color' => '#856404',
                'bg_color' => '#fff3cd',
                'icon' => 'bi-pencil-square',
                'description' => 'Product is in draft and not visible to customers',
                'order' => 2,
                'is_active' => true,
                'is_default' => false,
                'is_final' => false,
            ],
            [
                'module' => 'products',
                'name' => 'Out of Stock',
                'key_code' => 'PRODUCT_OUT_OF_STOCK',
                'slug' => 'out-of-stock',
                'color' => '#ffffff',
                'bg_color' => '#dc3545',
                'icon' => 'bi-x-circle-fill',
                'description' => 'Product is out of stock',
                'order' => 3,
                'is_active' => true,
                'is_default' => false,
                'is_final' => false,
                'send_email' => true,
                'email_template' => 'product_out_of_stock',
            ],
            [
                'module' => 'products',
                'name' => 'Discontinued',
                'key_code' => 'PRODUCT_DISCONTINUED',
                'slug' => 'discontinued',
                'color' => '#ffffff',
                'bg_color' => '#6c757d',
                'icon' => 'bi-archive-fill',
                'description' => 'Product is discontinued and no longer available',
                'order' => 4,
                'is_active' => true,
                'is_default' => false,
                'is_final' => true,
            ],

            // ============================================
            // PRODUCTS VARIENT MODULE STATUSES
            // ============================================
            [
                'module' => 'product_variants',
                'name' => 'Active',
                'key_code' => 'VARIANT_ACTIVE',
                'slug' => 'active',
                'color' => '#ffffff',
                'bg_color' => '#28a745',
                'icon' => 'bi-check-circle-fill',
                'description' => 'Product variant is active and available for purchase',
                'order' => 1,
                'is_active' => true,
                'is_default' => true,
                'is_final' => false,
            ],
            [
                'module' => 'product_variants',
                'name' => 'Inactive',
                'key_code' => 'VARIANT_INACTIVE',
                'slug' => 'inactive',
                'color' => '#ffffff',
                'bg_color' => '#6c757d',
                'icon' => 'bi-dash-circle-fill',
                'description' => 'Product variant is inactive and hidden from customers',
                'order' => 2,
                'is_active' => true,
                'is_default' => false,
                'is_final' => false,
            ],
            [
                'module' => 'product_variants',
                'name' => 'Out of Stock',
                'key_code' => 'VARIANT_OUT_OF_STOCK',
                'slug' => 'out-of-stock',
                'color' => '#ffffff',
                'bg_color' => '#dc3545',
                'icon' => 'bi-x-circle-fill',
                'description' => 'Product variant is out of stock',
                'order' => 3,
                'is_active' => true,
                'is_default' => false,
                'is_final' => false,
                'send_email' => true,
                'email_template' => 'variant_out_of_stock',
            ],

            // ============================================
            // ORDERS MODULE STATUSES
            // ============================================
            [
                'module' => 'orders',
                'name' => 'Pending',
                'key_code' => 'ORDER_PENDING',
                'slug' => 'pending',
                'color' => '#856404',
                'bg_color' => '#fff3cd',
                'icon' => 'bi-clock-fill',
                'description' => 'Order received and awaiting processing',
                'order' => 1,
                'is_active' => true,
                'is_default' => true,
                'is_final' => false,
                'allowed_transitions' => ['ORDER_CONFIRMED', 'ORDER_CANCELLED'],
                'send_email' => true,
                'email_template' => 'order_pending',
            ],
            [
                'module' => 'orders',
                'name' => 'Confirmed',
                'key_code' => 'ORDER_CONFIRMED',
                'slug' => 'confirmed',
                'color' => '#004085',
                'bg_color' => '#cce5ff',
                'icon' => 'bi-check2-circle',
                'description' => 'Order confirmed and being prepared',
                'order' => 2,
                'is_active' => true,
                'is_default' => false,
                'is_final' => false,
                'allowed_transitions' => ['ORDER_PROCESSING', 'ORDER_CANCELLED'],
                'send_email' => true,
                'email_template' => 'order_confirmed',
            ],
            [
                'module' => 'orders',
                'name' => 'Processing',
                'key_code' => 'ORDER_PROCESSING',
                'slug' => 'processing',
                'color' => '#0c5460',
                'bg_color' => '#d1ecf1',
                'icon' => 'bi-gear-fill',
                'description' => 'Order is being processed',
                'order' => 3,
                'is_active' => true,
                'is_default' => false,
                'is_final' => false,
                'allowed_transitions' => ['ORDER_SHIPPED', 'ORDER_CANCELLED'],
            ],
            [
                'module' => 'orders',
                'name' => 'Shipped',
                'key_code' => 'ORDER_SHIPPED',
                'slug' => 'shipped',
                'color' => '#383d41',
                'bg_color' => '#d6d8db',
                'icon' => 'bi-truck',
                'description' => 'Order has been shipped',
                'order' => 4,
                'is_active' => true,
                'is_default' => false,
                'is_final' => false,
                'allowed_transitions' => ['ORDER_DELIVERED', 'ORDER_RETURNED'],
                'send_email' => true,
                'email_template' => 'order_shipped',
                'send_notification' => true,
            ],
            [
                'module' => 'orders',
                'name' => 'Delivered',
                'key_code' => 'ORDER_DELIVERED',
                'slug' => 'delivered',
                'color' => '#ffffff',
                'bg_color' => '#28a745',
                'icon' => 'bi-check-circle-fill',
                'description' => 'Order has been delivered successfully',
                'order' => 5,
                'is_active' => true,
                'is_default' => false,
                'is_final' => true,
                'send_email' => true,
                'email_template' => 'order_delivered',
                'send_notification' => true,
            ],
            [
                'module' => 'orders',
                'name' => 'Cancelled',
                'key_code' => 'ORDER_CANCELLED',
                'slug' => 'cancelled',
                'color' => '#ffffff',
                'bg_color' => '#dc3545',
                'icon' => 'bi-x-circle-fill',
                'description' => 'Order has been cancelled',
                'order' => 6,
                'is_active' => true,
                'is_default' => false,
                'is_final' => true,
                'send_email' => true,
                'email_template' => 'order_cancelled',
            ],
            [
                'module' => 'orders',
                'name' => 'Returned',
                'key_code' => 'ORDER_RETURNED',
                'slug' => 'returned',
                'color' => '#721c24',
                'bg_color' => '#f8d7da',
                'icon' => 'bi-arrow-return-left',
                'description' => 'Order has been returned',
                'order' => 7,
                'is_active' => true,
                'is_default' => false,
                'is_final' => true,
                'send_email' => true,
                'email_template' => 'order_returned',
            ],

            // ============================================
            // ORDER ITEMS MODULE STATUSES
            // ============================================
            [
                'module' => 'order_items',
                'name' => 'Pending',
                'key_code' => 'ITEM_PENDING',
                'slug' => 'pending',
                'color' => '#856404',
                'bg_color' => '#fff3cd',
                'icon' => 'bi-clock-fill',
                'description' => 'Item is pending processing',
                'order' => 1,
                'is_active' => true,
                'is_default' => true,
                'is_final' => false,
            ],
            [
                'module' => 'order_items',
                'name' => 'Processing',
                'key_code' => 'ITEM_PROCESSING',
                'slug' => 'processing',
                'color' => '#004085',
                'bg_color' => '#cce5ff',
                'icon' => 'bi-gear-fill',
                'description' => 'Item is being processed',
                'order' => 2,
                'is_active' => true,
                'is_default' => false,
                'is_final' => false,
            ],
            [
                'module' => 'order_items',
                'name' => 'Fulfilled',
                'key_code' => 'ITEM_FULFILLED',
                'slug' => 'fulfilled',
                'color' => '#ffffff',
                'bg_color' => '#28a745',
                'icon' => 'bi-check-circle-fill',
                'description' => 'Item has been fulfilled',
                'order' => 3,
                'is_active' => true,
                'is_default' => false,
                'is_final' => false,
            ],
            [
                'module' => 'order_items',
                'name' => 'Shipped',
                'key_code' => 'ITEM_SHIPPED',
                'slug' => 'shipped',
                'color' => '#383d41',
                'bg_color' => '#d6d8db',
                'icon' => 'bi-truck',
                'description' => 'Item has been shipped',
                'order' => 4,
                'is_active' => true,
                'is_default' => false,
                'is_final' => false,
            ],
            [
                'module' => 'order_items',
                'name' => 'Delivered',
                'key_code' => 'ITEM_DELIVERED',
                'slug' => 'delivered',
                'color' => '#ffffff',
                'bg_color' => '#28a745',
                'icon' => 'bi-check-circle-fill',
                'description' => 'Item has been delivered',
                'order' => 5,
                'is_active' => true,
                'is_default' => false,
                'is_final' => true,
            ],
            [
                'module' => 'order_items',
                'name' => 'Cancelled',
                'key_code' => 'ITEM_CANCELLED',
                'slug' => 'cancelled',
                'color' => '#ffffff',
                'bg_color' => '#dc3545',
                'icon' => 'bi-x-circle-fill',
                'description' => 'Item has been cancelled',
                'order' => 6,
                'is_active' => true,
                'is_default' => false,
                'is_final' => true,
            ],
            [
                'module' => 'order_items',
                'name' => 'Refunded',
                'key_code' => 'ITEM_REFUNDED',
                'slug' => 'refunded',
                'color' => '#721c24',
                'bg_color' => '#f8d7da',
                'icon' => 'bi-arrow-return-left',
                'description' => 'Item has been refunded',
                'order' => 7,
                'is_active' => true,
                'is_default' => false,
                'is_final' => true,
            ],

            // ============================================
            // CUSTOMERS MODULE STATUSES
            // ============================================
            [
                'module' => 'customers',
                'name' => 'Active',
                'key_code' => 'CUSTOMER_ACTIVE',
                'slug' => 'active',
                'color' => '#ffffff',
                'bg_color' => '#28a745',
                'icon' => 'bi-person-check-fill',
                'description' => 'Customer account is active',
                'order' => 1,
                'is_active' => true,
                'is_default' => true,
                'is_final' => false,
            ],
            [
                'module' => 'customers',
                'name' => 'Inactive',
                'key_code' => 'CUSTOMER_INACTIVE',
                'slug' => 'inactive',
                'color' => '#ffffff',
                'bg_color' => '#6c757d',
                'icon' => 'bi-person-dash-fill',
                'description' => 'Customer account is inactive',
                'order' => 2,
                'is_active' => true,
                'is_default' => false,
                'is_final' => false,
            ],
            [
                'module' => 'customers',
                'name' => 'Blocked',
                'key_code' => 'CUSTOMER_BLOCKED',
                'slug' => 'blocked',
                'color' => '#ffffff',
                'bg_color' => '#dc3545',
                'icon' => 'bi-person-x-fill',
                'description' => 'Customer account is blocked',
                'order' => 3,
                'is_active' => true,
                'is_default' => false,
                'is_final' => false,
                'required_permissions' => ['customers.block'],
            ],

            // ============================================
            // INVENTORY MODULE STATUSES
            // ============================================
            [
                'module' => 'inventory',
                'name' => 'In Stock',
                'key_code' => 'INVENTORY_IN_STOCK',
                'slug' => 'in-stock',
                'color' => '#ffffff',
                'bg_color' => '#28a745',
                'icon' => 'bi-box-seam-fill',
                'description' => 'Item is in stock',
                'order' => 1,
                'is_active' => true,
                'is_default' => true,
                'is_final' => false,
            ],
            [
                'module' => 'inventory',
                'name' => 'Low Stock',
                'key_code' => 'INVENTORY_LOW_STOCK',
                'slug' => 'low-stock',
                'color' => '#856404',
                'bg_color' => '#fff3cd',
                'icon' => 'bi-exclamation-triangle-fill',
                'description' => 'Item stock is running low',
                'order' => 2,
                'is_active' => true,
                'is_default' => false,
                'is_final' => false,
                'send_email' => true,
                'email_template' => 'inventory_low_stock',
            ],
            [
                'module' => 'inventory',
                'name' => 'Out of Stock',
                'key_code' => 'INVENTORY_OUT_OF_STOCK',
                'slug' => 'out-of-stock',
                'color' => '#ffffff',
                'bg_color' => '#dc3545',
                'icon' => 'bi-x-circle-fill',
                'description' => 'Item is out of stock',
                'order' => 3,
                'is_active' => true,
                'is_default' => false,
                'is_final' => false,
                'send_email' => true,
                'email_template' => 'inventory_out_of_stock',
            ],
            [
                'module' => 'inventory',
                'name' => 'Reserved',
                'key_code' => 'INVENTORY_RESERVED',
                'slug' => 'reserved',
                'color' => '#004085',
                'bg_color' => '#cce5ff',
                'icon' => 'bi-lock-fill',
                'description' => 'Item stock is reserved for pending orders',
                'order' => 4,
                'is_active' => true,
                'is_default' => false,
                'is_final' => false,
            ],

            // ============================================
            // PAYMENTS MODULE STATUSES
            // ============================================
            [
                'module' => 'payments',
                'name' => 'Pending',
                'key_code' => 'PAYMENT_PENDING',
                'slug' => 'pending',
                'color' => '#856404',
                'bg_color' => '#fff3cd',
                'icon' => 'bi-clock-fill',
                'description' => 'Payment is pending',
                'order' => 1,
                'is_active' => true,
                'is_default' => true,
                'is_final' => false,
            ],
            [
                'module' => 'payments',
                'name' => 'Processing',
                'key_code' => 'PAYMENT_PROCESSING',
                'slug' => 'processing',
                'color' => '#004085',
                'bg_color' => '#cce5ff',
                'icon' => 'bi-hourglass-split',
                'description' => 'Payment is being processed',
                'order' => 2,
                'is_active' => true,
                'is_default' => false,
                'is_final' => false,
            ],
            [
                'module' => 'payments',
                'name' => 'Paid',
                'key_code' => 'PAYMENT_PAID',
                'slug' => 'completed',
                'color' => '#ffffff',
                'bg_color' => '#28a745',
                'icon' => 'bi-check-circle-fill',
                'description' => 'Payment completed successfully',
                'order' => 3,
                'is_active' => true,
                'is_default' => false,
                'is_final' => true,
                'send_email' => true,
                'email_template' => 'payment_completed',
            ],
            [
                'module' => 'payments',
                'name' => 'Failed',
                'key_code' => 'PAYMENT_FAILED',
                'slug' => 'failed',
                'color' => '#ffffff',
                'bg_color' => '#dc3545',
                'icon' => 'bi-x-circle-fill',
                'description' => 'Payment failed',
                'order' => 4,
                'is_active' => true,
                'is_default' => false,
                'is_final' => true,
                'send_email' => true,
                'email_template' => 'payment_failed',
            ],
            [
                'module' => 'payments',
                'name' => 'Refunded',
                'key_code' => 'PAYMENT_REFUNDED',
                'slug' => 'refunded',
                'color' => '#721c24',
                'bg_color' => '#f8d7da',
                'icon' => 'bi-arrow-return-left',
                'description' => 'Payment has been refunded',
                'order' => 5,
                'is_active' => true,
                'is_default' => false,
                'is_final' => true,
                'send_email' => true,
                'email_template' => 'payment_refunded',
            ],

            // ============================================
            // TRANSACTIONS MODULE STATUSES
            // ============================================
            [
                'module' => 'transactions',
                'name' => 'Pending',
                'key_code' => 'TRANSACTION_PENDING',
                'slug' => 'pending',
                'color' => '#856404',
                'bg_color' => '#fff3cd',
                'icon' => 'bi-clock-history',
                'description' => 'Transaction is pending',
                'order' => 1,
                'is_active' => true,
                'is_default' => true,
                'is_final' => false,
            ],
            [
                'module' => 'transactions',
                'name' => 'Processing',
                'key_code' => 'TRANSACTION_PROCESSING',
                'slug' => 'processing',
                'color' => '#004085',
                'bg_color' => '#cce5ff',
                'icon' => 'bi-arrow-repeat',
                'description' => 'Transaction is being processed',
                'order' => 2,
                'is_active' => true,
                'is_default' => false,
                'is_final' => false,
            ],
            [
                'module' => 'transactions',
                'name' => 'Authorized',
                'key_code' => 'TRANSACTION_AUTHORIZED',
                'slug' => 'authorized',
                'color' => '#004085',
                'bg_color' => '#d1ecf1',
                'icon' => 'bi-shield-check',
                'description' => 'Transaction has been authorized',
                'order' => 3,
                'is_active' => true,
                'is_default' => false,
                'is_final' => false,
            ],
            [
                'module' => 'transactions',
                'name' => 'Success',
                'key_code' => 'TRANSACTION_SUCCESS',
                'slug' => 'success',
                'color' => '#ffffff',
                'bg_color' => '#28a745',
                'icon' => 'bi-check-circle-fill',
                'description' => 'Transaction completed successfully',
                'order' => 4,
                'is_active' => true,
                'is_default' => false,
                'is_final' => true,
                'send_email' => true,
                'email_template' => 'transaction_success',
            ],
            [
                'module' => 'transactions',
                'name' => 'Failed',
                'key_code' => 'TRANSACTION_FAILED',
                'slug' => 'failed',
                'color' => '#ffffff',
                'bg_color' => '#dc3545',
                'icon' => 'bi-x-circle-fill',
                'description' => 'Transaction failed',
                'order' => 5,
                'is_active' => true,
                'is_default' => false,
                'is_final' => true,
                'send_email' => true,
                'email_template' => 'transaction_failed',
            ],
            [
                'module' => 'transactions',
                'name' => 'Cancelled',
                'key_code' => 'TRANSACTION_CANCELLED',
                'slug' => 'cancelled',
                'color' => '#383d41',
                'bg_color' => '#d6d8db',
                'icon' => 'bi-slash-circle',
                'description' => 'Transaction was cancelled',
                'order' => 6,
                'is_active' => true,
                'is_default' => false,
                'is_final' => true,
            ],
            [
                'module' => 'transactions',
                'name' => 'Refunded',
                'key_code' => 'TRANSACTION_REFUNDED',
                'slug' => 'refunded',
                'color' => '#721c24',
                'bg_color' => '#f8d7da',
                'icon' => 'bi-arrow-return-left',
                'description' => 'Transaction has been refunded',
                'order' => 7,
                'is_active' => true,
                'is_default' => false,
                'is_final' => true,
                'send_email' => true,
                'email_template' => 'transaction_refunded',
            ],
            [
                'module' => 'transactions',
                'name' => 'Partially Refunded',
                'key_code' => 'TRANSACTION_PARTIALLY_REFUNDED',
                'slug' => 'partially-refunded',
                'color' => '#721c24',
                'bg_color' => '#f5c6cb',
                'icon' => 'bi-arrow-left-right',
                'description' => 'Transaction has been partially refunded',
                'order' => 8,
                'is_active' => true,
                'is_default' => false,
                'is_final' => false,
            ],
            [
                'module' => 'transactions',
                'name' => 'On Hold',
                'key_code' => 'TRANSACTION_ON_HOLD',
                'slug' => 'on-hold',
                'color' => '#856404',
                'bg_color' => '#ffeaa7',
                'icon' => 'bi-pause-circle',
                'description' => 'Transaction is on hold for review',
                'order' => 9,
                'is_active' => true,
                'is_default' => false,
                'is_final' => false,
            ],
            [
                'module' => 'transactions',
                'name' => 'Voided',
                'key_code' => 'TRANSACTION_VOIDED',
                'slug' => 'voided',
                'color' => '#383d41',
                'bg_color' => '#e2e3e5',
                'icon' => 'bi-ban',
                'description' => 'Transaction has been voided',
                'order' => 10,
                'is_active' => true,
                'is_default' => false,
                'is_final' => true,
            ],
            [
                'module' => 'transactions',
                'name' => 'Disputed',
                'key_code' => 'TRANSACTION_DISPUTED',
                'slug' => 'disputed',
                'color' => '#ffffff',
                'bg_color' => '#ff7675',
                'icon' => 'bi-exclamation-triangle-fill',
                'description' => 'Transaction is under dispute',
                'order' => 11,
                'is_active' => true,
                'is_default' => false,
                'is_final' => false,
            ],
            [
                'module' => 'transactions',
                'name' => 'Chargeback',
                'key_code' => 'TRANSACTION_CHARGEBACK',
                'slug' => 'chargeback',
                'color' => '#ffffff',
                'bg_color' => '#d63031',
                'icon' => 'bi-exclamation-octagon-fill',
                'description' => 'Transaction has been charged back',
                'order' => 12,
                'is_active' => true,
                'is_default' => false,
                'is_final' => true,
                'send_email' => true,
                'email_template' => 'transaction_chargeback',
            ],
            [
                'module' => 'transactions',
                'name' => 'Expired',
                'key_code' => 'TRANSACTION_EXPIRED',
                'slug' => 'expired',
                'color' => '#6c757d',
                'bg_color' => '#e9ecef',
                'icon' => 'bi-hourglass',
                'description' => 'Transaction has expired',
                'order' => 13,
                'is_active' => true,
                'is_default' => false,
                'is_final' => true,
            ],

            // ============================================
            // SHIPMENTS MODULE STATUSES
            // ============================================
            [
                'module' => 'shipments',
                'name' => 'Pending',
                'key_code' => 'SHIPMENT_PENDING',
                'slug' => 'pending',
                'color' => '#856404',
                'bg_color' => '#fff3cd',
                'icon' => 'bi-clock-fill',
                'description' => 'Shipment is pending and awaiting pickup',
                'order' => 1,
                'is_active' => true,
                'is_default' => true,
                'is_final' => false,
            ],
            [
                'module' => 'shipments',
                'name' => 'In Transit',
                'key_code' => 'SHIPMENT_IN_TRANSIT',
                'slug' => 'in-transit',
                'color' => '#004085',
                'bg_color' => '#cce5ff',
                'icon' => 'bi-truck',
                'description' => 'Shipment is in transit',
                'order' => 2,
                'is_active' => true,
                'is_default' => false,
                'is_final' => false,
                'send_notification' => true,
            ],
            [
                'module' => 'shipments',
                'name' => 'Out for Delivery',
                'key_code' => 'SHIPMENT_OUT_FOR_DELIVERY',
                'slug' => 'out-for-delivery',
                'color' => '#0c5460',
                'bg_color' => '#d1ecf1',
                'icon' => 'bi-geo-alt-fill',
                'description' => 'Shipment is out for delivery',
                'order' => 3,
                'is_active' => true,
                'is_default' => false,
                'is_final' => false,
                'send_email' => true,
                'email_template' => 'shipment_out_for_delivery',
                'send_notification' => true,
            ],
            [
                'module' => 'shipments',
                'name' => 'Delivered',
                'key_code' => 'SHIPMENT_DELIVERED',
                'slug' => 'delivered',
                'color' => '#ffffff',
                'bg_color' => '#28a745',
                'icon' => 'bi-check-circle-fill',
                'description' => 'Shipment has been delivered',
                'order' => 4,
                'is_active' => true,
                'is_default' => false,
                'is_final' => true,
                'send_email' => true,
                'email_template' => 'shipment_delivered',
                'send_notification' => true,
            ],
            [
                'module' => 'shipments',
                'name' => 'Failed Delivery',
                'key_code' => 'SHIPMENT_FAILED_DELIVERY',
                'slug' => 'failed-delivery',
                'color' => '#721c24',
                'bg_color' => '#f8d7da',
                'icon' => 'bi-exclamation-triangle-fill',
                'description' => 'Delivery attempt failed',
                'order' => 5,
                'is_active' => true,
                'is_default' => false,
                'is_final' => false,
                'send_email' => true,
                'email_template' => 'shipment_failed_delivery',
            ],
            [
                'module' => 'shipments',
                'name' => 'Returned',
                'key_code' => 'SHIPMENT_RETURNED',
                'slug' => 'returned',
                'color' => '#ffffff',
                'bg_color' => '#dc3545',
                'icon' => 'bi-arrow-return-left',
                'description' => 'Shipment has been returned',
                'order' => 6,
                'is_active' => true,
                'is_default' => false,
                'is_final' => true,
                'send_email' => true,
                'email_template' => 'shipment_returned',
            ],

            // ============================================
            // CONTACT US MODULE STATUSES
            // ============================================

            [
                'module' => 'contact_us',
                'name' => 'New',
                'key_code' => 'CONTACT_NEW',
                'slug' => 'new',
                'color' => '#004085',
                'bg_color' => '#cce5ff',
                'icon' => 'bi-envelope-fill',
                'description' => 'New contact message received',
                'order' => 1,
                'is_active' => true,
                'is_default' => true,
                'is_final' => false,
                'send_notification' => true,
                'created_at' => now(),
                'updated_at' => now(),
            ],
            [
                'module' => 'contact_us',
                'name' => 'Pending',
                'key_code' => 'CONTACT_PENDING',
                'slug' => 'pending',
                'color' => '#856404',
                'bg_color' => '#fff3cd',
                'icon' => 'bi-clock-fill',
                'description' => 'Contact message is pending review',
                'order' => 2,
                'is_active' => true,
                'is_default' => false,
                'is_final' => false,
                'created_at' => now(),
                'updated_at' => now(),
            ],
            [
                'module' => 'contact_us',
                'name' => 'In Progress',
                'key_code' => 'CONTACT_IN_PROGRESS',
                'slug' => 'in-progress',
                'color' => '#0c5460',
                'bg_color' => '#d1ecf1',
                'icon' => 'bi-hourglass-split',
                'description' => 'Contact message is being processed',
                'order' => 3,
                'is_active' => true,
                'is_default' => false,
                'is_final' => false,
                'send_notification' => true,
                'created_at' => now(),
                'updated_at' => now(),
            ],
            [
                'module' => 'contact_us',
                'name' => 'Awaiting Response',
                'key_code' => 'CONTACT_AWAITING_RESPONSE',
                'slug' => 'awaiting-response',
                'color' => '#533f03',
                'bg_color' => '#ffeaa7',
                'icon' => 'bi-reply-fill',
                'description' => 'Waiting for customer response',
                'order' => 4,
                'is_active' => true,
                'is_default' => false,
                'is_final' => false,
                'send_email' => true,
                'email_template' => 'contact_awaiting_response',
                'created_at' => now(),
                'updated_at' => now(),
            ],
            [
                'module' => 'contact_us',
                'name' => 'Resolved',
                'key_code' => 'CONTACT_RESOLVED',
                'slug' => 'resolved',
                'color' => '#ffffff',
                'bg_color' => '#28a745',
                'icon' => 'bi-check-circle-fill',
                'description' => 'Contact message has been resolved',
                'order' => 5,
                'is_active' => true,
                'is_default' => false,
                'is_final' => true,
                'send_email' => true,
                'email_template' => 'contact_resolved',
                'send_notification' => true,
                'created_at' => now(),
                'updated_at' => now(),
            ],
            [
                'module' => 'contact_us',
                'name' => 'Closed',
                'key_code' => 'CONTACT_CLOSED',
                'slug' => 'closed',
                'color' => '#ffffff',
                'bg_color' => '#6c757d',
                'icon' => 'bi-x-circle-fill',
                'description' => 'Contact message has been closed',
                'order' => 6,
                'is_active' => true,
                'is_default' => false,
                'is_final' => true,
                'created_at' => now(),
                'updated_at' => now(),
            ],
            [
                'module' => 'contact_us',
                'name' => 'Spam',
                'key_code' => 'CONTACT_SPAM',
                'slug' => 'spam',
                'color' => '#721c24',
                'bg_color' => '#f8d7da',
                'icon' => 'bi-trash-fill',
                'description' => 'Marked as spam or unwanted message',
                'order' => 7,
                'is_active' => true,
                'is_default' => false,
                'is_final' => true,
                'created_at' => now(),
                'updated_at' => now(),
            ],
            [
                'module' => 'contact_us',
                'name' => 'Follow Up Required',
                'key_code' => 'CONTACT_FOLLOW_UP',
                'slug' => 'follow-up-required',
                'color' => '#8b4513',
                'bg_color' => '#ffe4b5',
                'icon' => 'bi-arrow-repeat',
                'description' => 'Requires follow up action',
                'order' => 8,
                'is_active' => true,
                'is_default' => false,
                'is_final' => false,
                'send_notification' => true,
                'created_at' => now(),
                'updated_at' => now(),
            ],
            [
                'module' => 'contact_us',
                'name' => 'Escalated',
                'key_code' => 'CONTACT_ESCALATED',
                'slug' => 'escalated',
                'color' => '#ffffff',
                'bg_color' => '#dc3545',
                'icon' => 'bi-arrow-up-circle-fill',
                'description' => 'Escalated to higher authority',
                'order' => 9,
                'is_active' => true,
                'is_default' => false,
                'is_final' => false,
                'send_email' => true,
                'email_template' => 'contact_escalated',
                'send_notification' => true,
                'created_at' => now(),
                'updated_at' => now(),
            ],
            [
                'module' => 'contact_us',
                'name' => 'On Hold',
                'key_code' => 'CONTACT_ON_HOLD',
                'slug' => 'on-hold',
                'color' => '#464646',
                'bg_color' => '#e2e3e5',
                'icon' => 'bi-pause-circle-fill',
                'description' => 'Contact message is on hold',
                'order' => 10,
                'is_active' => true,
                'is_default' => false,
                'is_final' => false,
                'created_at' => now(),
                'updated_at' => now(),
            ],
            // Active Status
            [
                'module' => 'vendors',
                'name' => 'Active',
                'key_code' => 'VENDOR_ACTIVE',
                'slug' => 'active',
                'color' => '#5B914C',
                'bg_color' => '#d4edda',
                'icon' => 'bi-check-circle-fill',
                'description' => 'Vendor is active and operational',
                'order' => 10,
                'is_active' => true,
                'is_default' => true,
                'is_final' => false,
                'created_at' => now(),
                'updated_at' => now(),
            ],

            // Inactive Status
            [
                'module' => 'vendors',
                'name' => 'Inactive',
                'key_code' => 'VENDOR_INACTIVE',
                'slug' => 'inactive',
                'color' => '#6c757d',
                'bg_color' => '#e2e3e5',
                'icon' => 'bi-x-circle-fill',
                'description' => 'Vendor is inactive and not accepting orders',
                'order' => 20,
                'is_active' => true,
                'is_default' => false,
                'is_final' => false,
                'created_at' => now(),
                'updated_at' => now(),
            ],

            // Pending Status
            [
                'module' => 'vendors',
                'name' => 'Pending',
                'key_code' => 'VENDOR_PENDING',
                'slug' => 'pending',
                'color' => '#ffc107',
                'bg_color' => '#fff3cd',
                'icon' => 'bi-clock-fill',
                'description' => 'Vendor registration is pending approval',
                'order' => 30,
                'is_active' => true,
                'is_default' => false,
                'is_final' => false,
                'created_at' => now(),
                'updated_at' => now(),
            ],

            // Suspended Status
            [
                'module' => 'vendors',
                'name' => 'Suspended',
                'key_code' => 'VENDOR_SUSPENDED',
                'slug' => 'suspended',
                'color' => '#dc3545',
                'bg_color' => '#f8d7da',
                'icon' => 'bi-exclamation-triangle-fill',
                'description' => 'Vendor account is suspended',
                'order' => 40,
                'is_active' => true,
                'is_default' => false,
                'is_final' => false,
                'created_at' => now(),
                'updated_at' => now(),
            ],

            // Under Review Status
            [
                'module' => 'vendors',
                'name' => 'Under Review',
                'key_code' => 'VENDOR_UNDER_REVIEW',
                'slug' => 'under-review',
                'color' => '#17a2b8',
                'bg_color' => '#d1ecf1',
                'icon' => 'bi-search',
                'description' => 'Vendor is under review by admin',
                'order' => 50,
                'is_active' => true,
                'is_default' => false,
                'is_final' => false,
                'created_at' => now(),
                'updated_at' => now(),
            ],

            // Blocked Status
            [
                'module' => 'vendors',
                'name' => 'Blocked',
                'key_code' => 'VENDOR_BLOCKED',
                'slug' => 'blocked',
                'color' => '#721c24',
                'bg_color' => '#f8d7da',
                'icon' => 'bi-ban',
                'description' => 'Vendor is permanently blocked',
                'order' => 60,
                'is_active' => true,
                'is_default' => false,
                'is_final' => true,
                'created_at' => now(),
                'updated_at' => now(),
            ],

            // On Hold Status
            [
                'module' => 'vendors',
                'name' => 'On Hold',
                'key_code' => 'VENDOR_ON_HOLD',
                'slug' => 'on-hold',
                'color' => '#464646',
                'bg_color' => '#e2e3e5',
                'icon' => 'bi-pause-circle-fill',
                'description' => 'Vendor operations are temporarily on hold',
                'order' => 70,
                'is_active' => true,
                'is_default' => false,
                'is_final' => false,
                'created_at' => now(),
                'updated_at' => now(),
            ],

            // Approved Status
            [
                'module' => 'vendors',
                'name' => 'Approved',
                'key_code' => 'VENDOR_APPROVED',
                'slug' => 'approved',
                'color' => '#28a745',
                'bg_color' => '#d4edda',
                'icon' => 'bi-check2-circle',
                'description' => 'Vendor application has been approved',
                'order' => 80,
                'is_active' => true,
                'is_default' => false,
                'is_final' => false,
                'created_at' => now(),
                'updated_at' => now(),
            ],

            // Rejected Status
            [
                'module' => 'vendors',
                'name' => 'Rejected',
                'key_code' => 'VENDOR_REJECTED',
                'slug' => 'rejected',
                'color' => '#e3342f',
                'bg_color' => '#f8d7da',
                'icon' => 'bi-x-octagon-fill',
                'description' => 'Vendor application has been rejected',
                'order' => 90,
                'is_active' => true,
                'is_default' => false,
                'is_final' => true,
                'created_at' => now(),
                'updated_at' => now(),
            ],

            // Draft Status
            [
                'module' => 'purchase_orders',
                'name' => 'Draft',
                'key_code' => 'PO_DRAFT',
                'slug' => 'draft',
                'color' => '#6c757d',
                'bg_color' => '#e2e3e5',
                'icon' => 'bi-file-earmark',
                'description' => 'Purchase order is in draft status',
                'order' => 10,
                'is_active' => true,
                'is_default' => true,
                'is_final' => false,
                'created_at' => now(),
                'updated_at' => now(),
            ],

            // Pending Status
            [
                'module' => 'purchase_orders',
                'name' => 'Pending',
                'key_code' => 'PO_PENDING',
                'slug' => 'pending',
                'color' => '#ffc107',
                'bg_color' => '#fff3cd',
                'icon' => 'bi-clock-fill',
                'description' => 'Purchase order is pending approval',
                'order' => 20,
                'is_active' => true,
                'is_default' => false,
                'is_final' => false,
                'created_at' => now(),
                'updated_at' => now(),
            ],

            // Approved Status
            [
                'module' => 'purchase_orders',
                'name' => 'Approved',
                'key_code' => 'PO_APPROVED',
                'slug' => 'approved',
                'color' => '#17a2b8',
                'bg_color' => '#d1ecf1',
                'icon' => 'bi-check-circle-fill',
                'description' => 'Purchase order has been approved',
                'order' => 30,
                'is_active' => true,
                'is_default' => false,
                'is_final' => false,
                'created_at' => now(),
                'updated_at' => now(),
            ],

            // In Progress Status
            [
                'module' => 'purchase_orders',
                'name' => 'In Progress',
                'key_code' => 'PO_IN_PROGRESS',
                'slug' => 'in-progress',
                'color' => '#5B914C',
                'bg_color' => '#d4edda',
                'icon' => 'bi-arrow-repeat',
                'description' => 'Purchase order is being processed',
                'order' => 40,
                'is_active' => true,
                'is_default' => false,
                'is_final' => false,
                'created_at' => now(),
                'updated_at' => now(),
            ],

            // Partially Received Status
            [
                'module' => 'purchase_orders',
                'name' => 'Partially Received',
                'key_code' => 'PO_PARTIAL',
                'slug' => 'partially-received',
                'color' => '#fd7e14',
                'bg_color' => '#ffe5d0',
                'icon' => 'bi-box-seam',
                'description' => 'Purchase order is partially received',
                'order' => 50,
                'is_active' => true,
                'is_default' => false,
                'is_final' => false,
                'created_at' => now(),
                'updated_at' => now(),
            ],

            // Completed Status
            [
                'module' => 'purchase_orders',
                'name' => 'Completed',
                'key_code' => 'PO_COMPLETED',
                'slug' => 'completed',
                'color' => '#28a745',
                'bg_color' => '#d4edda',
                'icon' => 'bi-check2-circle',
                'description' => 'Purchase order is completed',
                'order' => 60,
                'is_active' => true,
                'is_default' => false,
                'is_final' => true,
                'created_at' => now(),
                'updated_at' => now(),
            ],

            // Cancelled Status
            [
                'module' => 'purchase_orders',
                'name' => 'Cancelled',
                'key_code' => 'PO_CANCELLED',
                'slug' => 'cancelled',
                'color' => '#dc3545',
                'bg_color' => '#f8d7da',
                'icon' => 'bi-x-circle-fill',
                'description' => 'Purchase order has been cancelled',
                'order' => 70,
                'is_active' => true,
                'is_default' => false,
                'is_final' => true,
                'created_at' => now(),
                'updated_at' => now(),
            ],

            // On Hold Status
            [
                'module' => 'purchase_orders',
                'name' => 'On Hold',
                'key_code' => 'PO_ON_HOLD',
                'slug' => 'on-hold',
                'color' => '#464646',
                'bg_color' => '#e2e3e5',
                'icon' => 'bi-pause-circle-fill',
                'description' => 'Purchase order is on hold',
                'order' => 80,
                'is_active' => true,
                'is_default' => false,
                'is_final' => false,
                'created_at' => now(),
                'updated_at' => now(),
            ],
        ];

        // Insert or update each status
        foreach ($allStatuses as $statusData) {
            // Ensure array fields are properly handled
            $statusData['allowed_transitions'] = isset($statusData['allowed_transitions'])
                ? json_encode($statusData['allowed_transitions'])
                : null;

            $statusData['required_permissions'] = isset($statusData['required_permissions'])
                ? json_encode($statusData['required_permissions'])
                : null;

            $statusData['automation_triggers'] = isset($statusData['automation_triggers'])
                ? json_encode($statusData['automation_triggers'])
                : null;

            $statusData['metadata'] = isset($statusData['metadata'])
                ? json_encode($statusData['metadata'])
                : null;

            // Check if status already exists by key_code
            $existingStatus = SystemStatus::where('key_code', $statusData['key_code'])->first();

            if ($existingStatus) {
                // Update existing status
                $existingStatus->update($statusData);
                $this->command->info("✓ Updated: {$statusData['key_code']}");
            } else {
                // Create new status with UUID
                SystemStatus::create(array_merge($statusData, ['id' => Str::uuid()]));
                $this->command->info("✓ Created: {$statusData['key_code']}");
            }
        }

        $this->command->info('');
        $this->command->info('===========================================');
        $this->command->info('System Status Seeder Completed Successfully');
        $this->command->info('===========================================');
        $this->command->info('Total Statuses: ' . count($allStatuses));
        $this->command->info('');
    }
}
